...
def test_projects_relationships_success(mock_filter_by, client):
    ...
    mock_project = Project(neutral_id="01DB2ECBP24NHYV5KZQG2N3FS2")  # Example, adjust as needed
    ...
    with patch('arctic_office_projects_api.schemas.ProjectSchema.dump') as mock_dump:
        ...
        response = client.get("/projects/01DB2ECBP24NHYV5KZQG2N3FS2/relationships/participants")        
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}

        response = client.get("/projects/01DB2ECBP24NHYV5KZQG2N3FS2/relationships/allocations")        
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}
        ...
        response = client.get("/projects/01DB2ECBP24NHYV5KZQG2N3FS2/relationships/categorisations")        
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}
...
def test_grants_relationships_success(mock_filter_by, client):
    # Mock a project object and its schema output
    mock_project = Grant(neutral_id="01DB2ECBP3XQ4B8Z5DW7W963YD")  # Example, adjust as needed
    mock_filter_by.return_value.one.return_value = mock_project
    
    with patch('arctic_office_projects_api.schemas.GrantSchema.dump') as mock_dump:
        mock_dump.return_value = {"data": "expected_payload"}

        response = client.get("/grants/01DB2ECBP3XQ4B8Z5DW7W963YD/relationships/allocations")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}

        response = client.get("/grants/01DB2ECBP3XQ4B8Z5DW7W963YD/relationships/organisations")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}
...
def test_people_relationships_success(mock_filter_by, client):
    # Mock a project object and its schema output
    mock_project = Person(neutral_id="01DB2ECBP2MFB0DH3EF3PH74R0")  # Example, adjust as needed
    mock_filter_by.return_value.one.return_value = mock_project

    with patch('arctic_office_projects_api.schemas.PersonSchema.dump') as mock_dump:
        mock_dump.return_value = {"data": "expected_payload"}

        response = client.get("/people/01DB2ECBP2MFB0DH3EF3PH74R0/relationships/participants")        
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}

        response = client.get("/people/01DB2ECBP2MFB0DH3EF3PH74R0/relationships/organisations")        
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}
...
def test_participants_relationships_projects(mock_filter_by, client):
    # Mock a project object and its schema output
    mock_project = Participant(neutral_id="01DB2ECBP3622SPB5PS3J8W4XF")  # Example, adjust as needed
    mock_filter_by.return_value.one.return_value = mock_project
    
    with patch('arctic_office_projects_api.schemas.ParticipantSchema.dump') as mock_dump:
        mock_dump.return_value = {"data": "expected_payload"}

        response = client.get("/participants/01DB2ECBP3622SPB5PS3J8W4XF/relationships/projects")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}

        response = client.get("/participants/01DB2ECBP3622SPB5PS3J8W4XF/relationships/people")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}
...
def test_organisations_relationships_success(mock_filter_by, client):
    # Mock a project object and its schema output
    mock_project = Organisation(neutral_id="01DB2ECBP3WZDP4PES64XKXJ1A")  # Example, adjust as needed
    mock_filter_by.return_value.one.return_value = mock_project
    
    with patch('arctic_office_projects_api.schemas.OrganisationSchema.dump') as mock_dump:
        mock_dump.return_value = {"data": "expected_payload"}

        response = client.get("/organisations/01DB2ECBP3WZDP4PES64XKXJ1A/relationships/people")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}

        response = client.get("/organisations/01DB2ECBP3WZDP4PES64XKXJ1A/relationships/grants")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}
...
def test_allocations_relationships_success(mock_filter_by, client):
    # Mock a project object and its schema output
    mock_project = Allocation(neutral_id="01DB2ECBP35AT5WBG092J5GDQ9")  # Example, adjust as needed
    mock_filter_by.return_value.one.return_value = mock_project
    
    with patch('arctic_office_projects_api.schemas.AllocationSchema.dump') as mock_dump:
        mock_dump.return_value = {"data": "expected_payload"}

        response = client.get("/allocations/01DB2ECBP35AT5WBG092J5GDQ9/relationships/projects")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}

        response = client.get("/allocations/01DB2ECBP35AT5WBG092J5GDQ9/relationships/grants")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}
...
def test_categorisations_relationships_success(mock_filter_by, client):
    # Mock a project object and its schema output
    mock_project = Categorisation(neutral_id="01DC6HYAKYAXE7MZMD08QV5JWG")  # Example, adjust as needed
    mock_filter_by.return_value.one.return_value = mock_project
    
    with patch('arctic_office_projects_api.schemas.CategorisationSchema.dump') as mock_dump:
        mock_dump.return_value = {"data": "expected_payload"}

        response = client.get("/categorisations/01DC6HYAKYAXE7MZMD08QV5JWG/relationships/projects")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}

        response = client.get("/categorisations/01DC6HYAKYAXE7MZMD08QV5JWG/relationships/categories")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}
...
def test_categoryschemes_relationships_categories(mock_filter_by, client):
    # Mock a project object and its schema output
    mock_project = CategoryScheme(neutral_id="01DC6HYAKXG8FCN63D7DH06W84")  # Example, adjust as needed
    mock_filter_by.return_value.one.return_value = mock_project
    
    with patch('arctic_office_projects_api.schemas.CategorySchemeSchema.dump') as mock_dump:
        mock_dump.return_value = {"data": "expected_payload"}

        response = client.get("/category-schemes/01DC6HYAKXG8FCN63D7DH06W84/relationships/categories")
        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}
...
def test_categoryterms_relationships_parent_categories(mock_filter_by, client):
    # Mock a project object and its schema output
    mock_project = CategoryTerm(neutral_id="01DC6HYAKX993ZK6YHCVWAE169")  # Example, adjust as needed
    mock_filter_by.return_value.one.return_value = mock_project
    
    with patch('arctic_office_projects_api.schemas.CategoryTermSchema.dump') as mock_dump:
        mock_dump.return_value = {"data": "expected_payload"}
        response = client.get("/categories/01DC6HYAKX993ZK6YHCVWAE169/relationships/parent-categories")

        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}

        mock_dump.return_value = {"data": "expected_payload"}
        response = client.get("/categories/01DC6HYAKX993ZK6YHCVWAE169/relationships/category-schemes")

        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}

        mock_dump.return_value = {"data": "expected_payload"}
        response = client.get("/categories/01DC6HYAKX993ZK6YHCVWAE169/relationships/categorisations")

        assert response.status_code == 200
        assert response.json == {"data": "expected_payload"}

        mock_dump.return_value = {"data": "expected_payload"}
        response = client.get("/categories/01DC6HYAKX993ZK6YHCVWAE169/parent-categories")
