...
def test_orcid_id(fake):
    ...
    orcid = fake.orcid_id()
    assert orcid.startswith(
        "https://fake.orcid.org/0000-"
    ), "orcid_id should start with 'https://fake.orcid.org/0000-'"
    parts = orcid.split("-")
    assert (
        len(parts) == 4
    ), "ORCID ID should be in the format 'https://fake.orcid.org/0000-xxxx-xxxx-xxxx'"
    assert all(
        part.isdigit() for part in parts[1:]
    ), "Each part of the ORCID ID should be numeric"
...
def test_avatar(fake):
    ...
    ), "Avatar URL should start with 'https://randomuser.me/api/portraits/'"

    # Check if it's either male or female
    assert any(
        gender in avatar for gender in ["men", "women"]
    ), "Avatar URL should contain 'men' or 'women'"

    # Ensure the numeric part is within range
    parts = avatar.split("/")
    number_part = parts[-1].replace(".jpg", "")
    assert (
        number_part.isdigit() and 1 <= int(number_part) <= 99
    ), "Avatar number should be between 1 and 99"


def test_avatar_male(fake):
    """Test the avatar_male function returns a valid male avatar URL."""
    avatar_male = fake.avatar_male()
    assert avatar_male.startswith(
        "https://randomuser.me/api/portraits/men/"
    ), "Avatar male URL should start with 'https://randomuser.me/api/portraits/men/'"
    ...
    assert (
        number_part.isdigit() and 1 <= int(number_part) <= 99
    ), "Male avatar number should be between 1 and 99"
...
def test_avatar_female(fake):
    ...
    assert avatar_female.startswith(
        "https://randomuser.me/api/portraits/women/"
    ), "Avatar female URL should start with 'https://randomuser.me/api/portraits/women/'"
    ...
    assert (
        number_part.isdigit() and 1 <= int(number_part) <= 99
    ), "Female avatar number should be between 1 and 99"
