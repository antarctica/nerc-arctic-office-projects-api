...
Non-production data for this API is generated by [Database seeding](#database-seeding), using fake, but realistic 
values.
...
[Snyk](https://app.snyk.io/org/antarctica/project/xxx) for vulnerabilities. 
...
Each [Database model](#database-models) should include a seeding method (conventionally `seed()`) that will seed 
realistic, or real data for that model. Calls to these methods should then be added to the `seed` Flask CLI command in
`manage.py` and the relevant `setUp()` method in integration tests.

It is recommended seeding includes a number of static, predictable, instances of each model to aid in testing. 

By default, the `seed` CLI command will generate a single instance, which should be a static instance. Where additional 
instances are requested using the `--count` option, random instances should be generated using tools such as 
[Faker](#faker).

To run database seeding manually, use the `seed` CLI command:

```shell
$ docker-compose run app flask seed --count 3
```

Where `--count` can be any value greater than 1, or omitted where only a single value is desired.

**Note:** The count value should be treated as a target number of resources to add, as Faker is unaware of unique 
...
`en-GB`). It is recommended to use Faker when seeding random model instances when seeding.
